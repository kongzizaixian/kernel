		       ...
		       struct domain_device *device)
{
	...
	if (!sata_dev) {
		 ...
		 * chip bug workaround. So we ignore the first 32 even iptts.
		 */
		start = 64 * (sata_idx + 1);
		end = 64 * (sata_idx + 2);
	}
...
static int parse_trans_tx_err_code_v2_hw(u32 err_msk)
{
	...
	const int err_bit_cnt = ARRAY_SIZE(trans_tx_err_code_prio);
	int index, i;
...
static int parse_trans_rx_err_code_v2_hw(u32 err_msk)
{
	...
	const int err_bit_cnt =	ARRAY_SIZE(trans_rx_err_code_prio);
	int index, i;
...
static int parse_dma_tx_err_code_v2_hw(u32 err_msk)
{
	...
	const int err_bit_cnt =	ARRAY_SIZE(dma_tx_err_code_prio);
	int index, i;
...
static int parse_sipc_rx_err_code_v2_hw(u32 err_msk)
{
	...
	const int err_bit_cnt =	ARRAY_SIZE(sipc_rx_err_code_prio);
	int index, i;
...
static int parse_dma_rx_err_code_v2_hw(u32 err_msk)
{
	...
	const int err_bit_cnt =	ARRAY_SIZE(dma_rx_err_code_prio);
	int index, i;
			   ...
			   int err_phase)
{
	...
	if (err_phase == 1) {
		/* error happens on TX phase, the priorty of error is: DW2 > DW0 */
		error = parse_dma_tx_err_code_v2_hw(dma_tx_err_type);
		if ( error == -1)
			error = parse_trans_tx_err_code_v2_hw(
					trans_tx_fail_type);
	} else if (err_phase == 2) {
		/* error happens on RX phase, the priory is: DW1 > DW3 > DW2 */
